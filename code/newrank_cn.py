...
def getTokensFromFolder(rootdir):
	...
	for i in range(0,len(list)):
			...
			if os.path.isfile(path):
					try:
						fo = open(path, 'r+')
						word_generator +=jieba.cut_for_search(' '.join(fo.readlines()))
					except:
...
# process generate word clouds
def proc(cat, type):
	imageFile = imagePath+'/'+type+'/'+cat+'.png'
	print('------------------------ ' + cat + ' ------------------------')
	rootdir = './'+cat+'/'+type
	#get words
	word_generator = getTokensFromFolder(rootdir)
	print(now()+' read lines number: '+str(len(word_generator)))
	#remove empty words and stop words
	words_list = [k for k in word_generator if len(k)>1 and k not in stopWords]
	print(now()+' words number: '+str(len(words_list)))
	_count = Counter(words_list)
	#cut dictionary
	_frequencies = cutDict(_count, maxWordsNum)
	print(now()+' cut frequencies to number: '+str(len(_frequencies)))
	#init word cloud
	'''
	back_color = imread(backgroundImage)
	wc = WordCloud(background_color='white',
				   max_words=maxWordsNum,
				   mask=back_color,
				   max_font_size=300,
	...
	#start generate word cloud
	print(now()+' will process generate_from_frequencies...')
	wc.generate_from_frequencies(_count)
	
	#recolor image base on background image
...
for l in range(0,len(baselist)):
		...
		if os.path.isdir(p):
				proc(os.path.basename(p), wechatArticleType)
t_end = time.time()

print(now()+' total finished.')