...
t_start = time.time()
stopWords = ['微信','二维码','二维','扫一','一扫','公众','赞赏','转账','关注','打开','阅读','图片','关闭','取消','程序','功能','公司','点击']+stopwords0
...
def proc(folder, type):
	jpgFile = imagePath+'/'+type+'_'+folder+'.jpg'
	print('will processjpg file: ' + jpgFile)
	rootdir = './'+folder+'/'+type
	word_generator = getTokensFromFolder(rootdir)
	print('read lines number: '+str(len(word_generator)))
	words_list = [k for k in word_generator if len(k)>1 and k not in stopWords]
	print('words number: '+str(len(words_list)))
	back_color = imread(backgroundImage)
	print('will process WordCloud init')
	...
	t_generate_s = time.time()
	print('will process generate_from_frequencies...')
	wc.generate_from_frequencies(_count)
	t_generate_e = time.time()
	print('spent time of generating word cloud: '+str(t_generate_e-t_generate_s)+' seconds.')
	wc.to_file(jpgFile)
...
t_end = time.time()
print('total cost time: '+str(t_end-t_start)+' seconds.')